...
import logging
import re
import yaml  # Importing yaml for reading YAML configuration files
...
def load_config():
    ...
    if os.path.exists('config.yaml'):
        with open('config.yaml', 'r') as config_file:
            config = yaml.safe_load(config_file)  # Load the YAML file
    return config

# Get the current timestamp in GMT+05:30
def get_timestamp():
...
def get_blaze_times_news(feed_url):
    try:
...
async def news(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    feed_url = config.get("FEED_URL", "https://www.theblazetimes.in/feed.xml")  # Default feed URL
    blaze_news_text = get_blaze_times_news(feed_url)
...
def main():
    config = load_config()  # Load the configuration
    TELEGRAM_BOT_TOKEN = config.get("TELEGRAM_TOKEN")  # Get token from config
    if not TELEGRAM_BOT_TOKEN:
        logging.error("No Telegram Token found in config. Exiting.")
        return
